...
- 선택된 회사의 메시지를 LLM-first 운영 planner로 보내는 전용 `운영 채팅` 탐색 surface 제공. planner는 가능한 경우 로컬 Ollama Gemma를 먼저 쓰고, 대충 쓴 자연어를 해석하고, 검증된 회사 도구를 고른 뒤, 실제 실행 결과를 근거로 답하므로 키워드 매칭형 고정 상태 문구로 떨어지지 않음
